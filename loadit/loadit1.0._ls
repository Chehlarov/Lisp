...
;;; NLIST.lis contains node list
  (if (setq fil
	     (open
	       "E:/NIKI (D)/Nikolay/work/Jonko/Makrosi_Autocad/lis/NLIST.lis"
	       "r"
	     )
      )
    (progn
	...
	(if (and (read lin) (= (type (read lin)) 'int))
	  (progn
	    ...
	    (setq lin (vl-string-left-trim " " lin))
	    (setq y (read lin))
	    (setq node (append node (list (list x y))))
	  )
	)
      )
      (close fil)
...
;;; getting information for elements and its nodes
  (if (setq fil
	     (open
	       "E:/NIKI (D)/Nikolay/work/Jonko/Makrosi_Autocad/lis/ELIST.lis"
	       "r"
	     )
      )
    (progn
...
;;; getting information for elements and its nodes
  (if (setq fil
	     (open
	       "E:/NIKI (D)/Nikolay/work/Jonko/Makrosi_Autocad/lis/PRETAB.lis"
	       "r"
	     )
      )
    (progn
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (while (<= i elemnum)
    ...
    (setq Mx (car (nth i mom)))
    (setq My (cadr (nth i mom)))
